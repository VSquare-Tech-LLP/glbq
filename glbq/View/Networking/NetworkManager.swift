...
final class NetworkManager {
    ...
    private let baseURL = "https://api.glbq.org"

    // POST /api/img-to-img  (multipart form-data)
    func uploadImages(venueImage: UIImage,
                      referenceImage: UIImage,
                      prompt: String) async throws -> ImageUploadResponse {
        ...
        // 1️⃣ First image in images[] array
        if let data = venueImage.jpegData(compressionQuality: 0.9) {
            body.append("--\(boundary)\r\n")
            body.append("Content-Disposition: form-data; name=\"images[]\"; filename=\"venue.jpg\"\r\n")
            body.append("Content-Type: image/jpeg\r\n\r\n")
            body.append(data)
            body.append("\r\n")
        }

        // 2️⃣ Second image in images[] array
        if let data = referenceImage.jpegData(compressionQuality: 0.9) {
            body.append("--\(boundary)\r\n")
    ...
    // POST /api/get-result  (multipart form-data with "id")
    ...
    // POST /api/img-to-img  (multipart form-data) — single image variant
    func uploadDesign(image: UIImage, prompt: String) async throws -> ImageUploadResponse {
        guard let url = URL(string: "\(baseURL)/api/img-to-img") else { throw NetworkError.invalidURL }
    ...
    // POST /api/txt-to-img (multipart/form-data)
